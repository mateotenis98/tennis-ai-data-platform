...
**Key lesson from Sprint 4:** `gemini-2.5-flash` with Google Search grounding returns 0 content parts (`response.text = None`) — a known SDK incompatibility with the thinking model. Remove the `GoogleSearch` tool and rely on model training data for rankings. Rankings update weekly but don't shift dramatically between sessions, making training data sufficient for this use case.
